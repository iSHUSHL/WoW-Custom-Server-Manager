# v1.6.46 — Dungeon Clear Runtime Deployment Fix

- Fixes the successful Dungeon Clear build being installed only under `sources/wotlk/core/env/dist` while WoWCC continued launching `runtime/profiles/wotlk/bin/worldserver`.
- After a successful module build, WoWCC now atomically deploys the rebuilt `worldserver` and `authserver` into the actual WotLK runtime profile.
- Copies the Dungeon Clear module config into the runtime module-config directory.
- SHA-256 verifies that the runtime `worldserver` is byte-for-byte the freshly rebuilt module-enabled binary.
- Server startup now logs the exact runtime and compatibility binary paths plus Dungeon Clear config presence.
- Restart World Server after installation; the next launch uses the verified Dungeon Clear binary.

# v1.6.45 — Double-Click Build Launcher Fix

- Preserves executable permissions for `Build.command` in the ZIP.
- Adds `BUILD-WoWCC.command` as a second Finder-friendly launcher.
- The fallback launcher restores executable permission on `Build.command` before starting it.

# v1.6.44 — AzerothCore GNU Readline Build Fix

- Fixes `CliRunnable.cpp` failures for missing `::rl_done` and `::rl_event_hook`.
- Dungeon Leader builder explicitly selects Homebrew GNU Readline instead of macOS/libedit compatibility headers.
- Clears stale CMake detection before configuring.
- Prints the selected Readline location and required header symbols before compilation.

# v1.6.43 — Swift 6.4 Dungeon Leader Build Fix

- Fixed Swift 6.4 strict-concurrency errors caused by `Task.detached` capturing `ServerModel`.
- Live Dungeon Bot Leader output now streams through `FileHandle.readabilityHandler`.
- UI and WoWCC log updates are marshalled onto the main queue.
- Live six-stage status remains enabled.

# v1.6.42 — Live Dungeon Leader Build Status

- Dungeon Bot Leader installation now streams stdout/stderr live into the WoWCC status panel.
- Added six clear stages: core detection, module download/update, CMake configure, compile, install, verification.
- CMake/Ninja compiler output is shown live instead of leaving the UI stuck on “Installing…”.
- Every streamed line is also mirrored into WoWCC diagnostic Logs.
- Persistent full build log remains at `runtime/dungeon-clear-build.log`.
- Clear ✓ COMPLETE / ✗ FAILED final state.

# v1.6.41 — Dungeon Bot Leader

- Added one-click **Install / Update Dungeon Bot Leader** for WotLK.
- Integrates upstream `jrad7/mod-dungeon-clear` with the existing AzerothCore PlayerBots core.
- WoWCC clones/updates the module, rebuilds worldserver/authserver with `MODULES=static`, installs the module config, and logs to `runtime/dungeon-clear-build.log`.
- After restart, enter a dungeon with a tank bot and run `.dc on`.
- Supported controls include `.dc off`, `.dc pause`, `.dc skip`, `.dc status`, `.dc bosses`, `.dc pull`, and `.dc go <boss>`.
- The module dynamically routes over the live navmesh and supports heroic difficulty.

# v1.6.40 — Gear Menu macOS Bash Fix

- Fixed `GEN[@]: unbound variable` in the one-click Gear Menu module build.
- Root cause: macOS Bash 3.2 + `set -u` does not safely expand an unset/empty optional array in this path.
- Removed the optional generator array completely.
- The script now uses an explicit Ninja CMake command when Ninja exists and a separate default-generator command otherwise.
- No need to reinstall the GM Island database before retrying the module build; click **Build / Update Gear Menu Module** again.

# v1.6.39 — One-Click Gear Menu Module Build

- Added **Build / Update Gear Menu Module** directly to GM Island Gear Hub.
- WoWCC installs its bundled AzerothCore module and rebuilds WotLK `worldserver`/`authserver` automatically.
- Uses `-j1` for Apple Silicon build stability.
- Build log: `runtime/wowcc-gear-terminal-build.log`, intended for WoWCC Logs.
- After COMPLETE: run GM Island Install/Repair and restart World Server.

# v1.6.38 — Gear Menu Empty-Window Fix

- Corrected the misleading central NPC behavior: the central NPC is now explicitly a **Gossip Gear Menu**, not a stock vendor.
- The actual item inventories remain on the seven nearby specialist vendor NPCs.
- Added clear installer diagnostics showing the Gear Menu `ScriptName` and `npcflag`.
- Added `check-wowcc-gear-terminal.sh` to verify the AzerothCore module source is installed before rebuilding.
- Improved Gossip text so a successfully loaded module visibly opens the WoWCC catalog menu.
- If the central NPC shows no Gossip menu, `mod-wowcc-gear-terminal` has not been compiled/loaded into the running worldserver; SQL alone cannot provide the class/spec menu.
- Run Install/Repair after the rebuilt worldserver is installed, then fully restart World Server.

# v1.6.37 — Class/Spec GM Gear Terminal

- Added source for a real AzerothCore custom Gossip NPC: `wowcc_gear_terminal`.
- Terminal menu covers all 10 WotLK classes and their 3 talent specs.
- Menu path: PvE/PvP → Class → Spec → Armor / Weapons / Accessories / Alternatives.
- Added central GM Island terminal spawn (entry 990120) plus the seven clean specialist catalogs.
- Added `Scripts/install-wowcc-gear-terminal-module.sh` to install the module source into an AzerothCore tree before rebuilding the core.
- The stock WotLK client cannot dynamically filter an ordinary vendor inventory from Gossip alone; this build therefore keeps catalog vendors as the actual item source instead of falsely claiming dynamic stock replacement.
- Run GM Hub Install/Repair after rebuilding the core/module, then restart World Server.

# v1.6.36 — Clean GM Specialist Hub

- Replaced the sprawling multi-page GM showroom with seven deliberately placed specialist NPCs.
- Install/Repair now deletes every older WoWCC GM Hub NPC/template/vendor entry in 990100–990299 before rebuilding the clean layout.
- Specialists: PvE Endgame, PvP Endgame, Weapons, Accessories, Enhancements, Collections, Utility.
- PvE/PvP armor, weapons, accessories, upgrades, mounts and utility are no longer mixed into dozens of NPC pages.
- Keeps the existing friendly/non-attackable/vendor interaction repair and installer verification.
- Standard AzerothCore/WotLK vendor UI has no native class/spec dropdown; WoWCC no longer presents the old layout as if it did.
- Run Install / Repair, wait for COMPLETE, then fully restart World Server.

# v1.6.35 — Complete Category GM Showroom

- Rebuilt GM Island around categories instead of one PvE/PvP NPC per class.
- Complete categories: PvE Sets, PvP Sets, PvE Weapons, PvP Weapons, PvE Accessories, PvP Accessories, Gems, Glyphs, Enchants/Enhancements, Mounts, Consumables and Bags.
- PvP includes Wrathful, Relentless and Furious Gladiator catalogs.
- PvE includes all epic/legendary level-80 item-level 245+ endgame armor/weapons/accessories from the installed AzerothCore DB.
- Mount catalog now includes every matching WotLK mount-learning item, not only rare mounts.
- Removed the old 145-results truncation. Large categories automatically paginate into numbered NPCs with 140 items each so every matching item remains visible without exceeding AzerothCore's per-vendor client safety limit.
- Reserved WoWCC vendor range expanded to 990100–990229.
- Run Install / Repair and restart World Server.

# v1.6.34 — GM Vendor Interaction Repair

- Corrected GM Island vendor interaction at both `creature_template` and spawn override level.
- Vendors are explicitly `npcflag=129` (Gossip + Vendor) with `IconName=Buy`.
- Vendors use faction 35 and are non-attackable, immune to players, and cannot enter combat.
- Clears spawn-level `npcflag`, `unit_flags`, and `dynamicflags` overrides because AzerothCore gives those precedence over the template.
- Added installer verification: installation now fails loudly if a WoWCC NPC lacks the Vendor flag or has an empty inventory.
- Run Install / Repair and fully restart World Server.

# v1.6.33 — Friendly GM Vendors

- Fixed GM Island vendors appearing hostile/attackable.
- Vendor templates now explicitly use friendly faction 35.
- NPC flags are explicitly Gossip + Vendor (129).
- Vendors are non-attackable and inherited hostile/dynamic flags are cleared.
- Run Install / Repair GM Island Hub again and restart World Server.

# v1.6.32 — GM Island MySQL Schema Fix

- Fixed GM Island vendor installation failing on AzerothCore/MySQL with `ERROR 1064` near the reserved `rank` column.
- Dynamic `creature_template` and `creature` INSERT column lists are now backtick-quoted, including MySQL reserved identifiers.
- Install / Repair remains idempotent; rerunning it safely rebuilds the WoWCC custom vendor range.
- GM Island PvE/PvP catalog and specialty vendors are unchanged.

# v1.6.31 — GM Island Build Fix

- Fixed the 1.6.30 Swift compile failure in `ServerModel.swift`.
- Removed invalid references to `selectedLogID` and `refreshSelectedLog()` from `ServerModel.openGMIslandInstallLog()`.
- The GM Island Gear Hub functionality remains unchanged.
- Existing compiler warnings are non-fatal and do not cause this build failure.

# v1.6.30 — GM Island Endgame Gear Hub

- Added **GM Island Gear Hub** for WotLK / AzerothCore.
- Installs/repairs 20 class vendors: separate PvE and PvP vendor for all 10 WotLK classes.
- PvE vendors dynamically pull epic/legendary ICC/RS-tier level-80 equipment from the installed AzerothCore item database.
- PvP vendors dynamically pull Wrathful Gladiator gear plus Relentless alternatives.
- Added shared vendors for weapons, trinkets/jewelry, PvP accessories, epic gems, glyphs, enchants, consumables, bags and rare mount items.
- Vendors are physically spawned on GM Island around map 1 / X 16222 / Y 16252 / Z 14.
- Installer is idempotent and reserves only creature entries 990100–990199.
- Each vendor is hard-limited to 145 items to remain below AzerothCore's 150-item client safety limit.
- Installer output is written to WoWCC `installer.log` and accessible from Logs.
- Restart World Server after install/repair to load the new templates/spawns.

# v1.6.29 — Quick Give

- Added a dedicated **Quick Give** page.
- Select a toon once; no need to type the character name.
- **Give Item**: enter Item ID + quantity and send it to the selected WotLK toon.
- **Teach Spell / Mount**: enter Spell ID and teach it to the selected toon.
- World Console now accepts `.additem ITEM [COUNT]` blocks and converts them to explicit selected-character commands.
- Quick Give and World Console actions are logged in WoWCC Logs.
- Existing AI Game Master, Character Maxer and Character Inventory remain removed.

# v1.6.27 — Targeted World Console

- Added a **World Console** page to WoWCC.
- Character picker at the top is the authoritative target.
- Paste a multi-line GM command block and click **Execute All**.
- For WotLK/AzerothCore, WoWCC safely converts common character commands to explicit named-player server commands:
  - `.learn SPELL` → `player learn CHARACTER SPELL`
  - `.unlearn SPELL` → `player unlearn CHARACTER SPELL`
  - `.level N` → `character level CHARACTER N`
  - `.levelup N` → `levelup CHARACTER N`
- Includes a one-click Riding + Rare Mount example block.
- Unsupported selection-dependent commands are rejected rather than accidentally applied to another player.
- All queue/send/failure activity is available from the existing WoWCC Logs diagnostics.
- AI Game Master, Character Maxer and Character Inventory remain removed.

# v1.6.26 — Remove Final AI References

- Fixed the remaining compile failure after AI Game Master removal.
- Removed stale Dashboard buttons for **AI Game Master**, **Best Gear**, **Rare Mounts**, and **Give Gear**.
- Removed the final `model.sendAIMessage(...)` references from `ContentView.swift`.
- Removed the stale AI sidebar icon mapping.
- Characters remains simplified to toon list/status/details + **Set Level** only.
- Character Inventory, AI Game Master and Character Maxer remain removed.

# v1.6.25 — Build Fix

- Fixed the v1.6.24 compile failure introduced during AI/Character Maxer removal.
- Restored the two private queue properties still required by the retained **Set Level** command bridge:
  - `adminCommandQueue`
  - `adminCommandWorkerRunning`
- AI Game Master, Character Maxer and Character Inventory remain removed.
- Characters remains intentionally simplified to toon status/details + Set Level.

# v1.6.24 — Characters Simplified

- Removed **AI Game Master** completely from the app.
- Removed **WotLK Character Maxer** completely.
- Removed **Character Inventory** from the Characters screen.
- Characters now keeps only the toon list, ONLINE/OFFLINE state, selected-character details, and **Set Level**.
- Kept the existing command bridge behind Set Level so the button can still communicate with the selected realm.
- PlayerBots, Accounts, Database, Logs, Backups, server controls and Settings are unchanged.

# v1.6.23 — Deterministic WotLK Character Maxer

- Added **WotLK Character Maxer** directly to Characters.
- This path bypasses ChatGPT and the World-server stdin console for core character changes.
- Requires the selected toon to be offline, then writes directly to `acore_characters`.
- Sets level 80, XP 0, 100,000 gold, full bank slots, and (when supported by schema) +200 bonus talent points.
- Sets common weapon/defense skills to 400/400.
- Learns riding, epic riding, flying, Cold Weather Flying, Dual Talent Specialization, plus verified class/race spells found in AzerothCore `playercreateinfo_spell_custom`.
- Sets major WotLK factions to Exalted.
- For WotLK Warlocks, directly mails a verified ICC/Ruby Sanctum end-game BIS pack through `item_instance` + `mail` + `mail_items`, avoiding the interactive World console entirely.
- Creates a pre-change character snapshot in WoWCC Backups.
- Adds an on-screen verification report and WoWCC-visible diagnostic entry.

# v1.6.22 — Command Execution Recovery

- Fixed the main reason AI/manual character commands could appear to succeed while doing nothing: after a WoWCC relaunch, an already-running World Server has no attachable stdin pipe. WoWCC now detects that condition, restarts only the selected profile's World process, reattaches its admin console, and then drains queued commands.
- Admin commands are serialized in a queue so multi-item AI grants are not lost during console recovery.
- Every queued/sent/failed command is written to WoWCC-visible diagnostics.
- WotLK Set Level now uses AzerothCore's absolute named-character console command: `character level <name> <level>`.
- AI level requests such as `set my toon to lvl 80` are recognized locally and executed immediately without relying on the model to construct an action.
- AI item verification now loops through up to four live realm-database search rounds instead of stopping after one extra verification pass.
- After commands run, Characters and Inventory refresh automatically.

# v1.6.21 — Live Toons + AI Auto-Execution

- Characters now has **Refresh Toons** and refreshes the human-character list every 3 seconds while the Characters page is visible, so ONLINE/OFFLINE state tracks the realm.
- Removed obsolete **Open Gear Catalog** and **Open Mount Catalog** buttons from Characters.
- AI Game Master now automatically executes returned character actions instead of merely preparing them and waiting for a separate Apply click.
- Imperative prompts (`give`, `set`, `learn`, `teleport`, `max`, etc.) are explicitly instructed to return executable actions immediately.
- Restored AzerothCore's documented named-player level command: `levelup <playerName> <delta>`; target name is always explicit.
- AI action summaries and execution diagnostics continue to be written to WoWCC-visible logs/status.

# v1.6.20 — AI Character Targeting Fix

- AI Game Master now treats the currently selected character as the mandatory target for every character-changing request.
- Fixed WotLK/AzerothCore level changes to use the named-character command `character level <name> <level>` instead of a delta-based `levelup` command.
- Structured AI actions for items, spells, and levels continue through WoWCC adapters that inject the selected character automatically.
- Raw AI commands that look character-mutating are rejected unless they explicitly contain the selected character name.
- Apply now snapshots the selected character and aborts/skips if the selection changes while actions are being executed.
- Character list refreshes automatically after level/action application so the new level is visible in WoWCC.

# v1.6.19 — Human Characters Only

- Characters page now filters PlayerBot/random-bot accounts for every supported core, including WotLK AzerothCore.
- Excludes `RNDBOT%`, `PLAYERBOT%`, and `BOT%` account prefixes from the Characters list.
- Bot population remains available in the dedicated PlayerBots section.
- AI Game Master character selection therefore uses the same human-only character list.

# v1.6.18 — AI Game Master Build Repair

- Fixed a Swift parser error in `AIService.swift` caused by an incorrectly escaped empty-string literal inside `AIGMAction.id`.
- Re-ran Swift frontend parsing across every Swift source file in the project.
- AI Game Master functionality from 1.6.17 is otherwise unchanged.

# v1.6.17 — AI Game Master

- Replaced the user-facing Gear Sets, Collection Browser, Weapons, Armor and Mount collection navigation with **AI Game Master**.
- Added conversational OpenAI Responses API integration with selectable GPT-5.6 Luna/Terra/Sol models.
- OpenAI API key is stored in macOS Keychain, not UserDefaults or project files.
- AI item recommendations use a two-pass workflow: request realm searches, query the selected expansion's live item database, then let AI choose only verified returned item IDs.
- Added action preview and explicit Apply button before WoWCC gives items, learns spells, changes level or sends GM console commands.
- Existing core adapters perform the actual in-game action: AzerothCore additem/player learn and CMaNGOS mail/direct DB spell handling.
- Added quick prompts for Best Gear, Best Weapons, Rare Mounts and Max Character.
- AI execution activity is written into WoWCC-visible diagnostic logs.

# v1.6.16 — UI Beachball / Main-Thread Freeze Fix

- Removed synchronous `/bin/ps` service scans and watchdog port probing from the automatic MainActor timer.
- Automatic monitoring now runs only the existing background-safe service/database probe.
- Expansion-wide process scanning now runs on a utility queue and publishes results back to SwiftUI asynchronously.
- Added a manual **Refresh** button to the top bar for an immediate status refresh without blocking the UI.
- Startup behavior from 1.6.15 remains: heavy monitoring begins only after the first window appears.
- Five-expansion scope remains Vanilla, TBC, WotLK, Cataclysm, and MoP.

# v1.6.15 — Dock Bounce / Startup Hardening

- Deferred process, port, database and service-state probing until after the first SwiftUI window is presented.
- Removed the unnecessary forced `NSPrincipalClass` key from the manually assembled SwiftUI app bundle.
- Build.command now strips inherited Gatekeeper quarantine from the locally built `.app`.
- Ad-hoc signing is now mandatory and immediately verified with `codesign --verify --deep --strict`.
- Build.command verifies the final executable is executable and is a Mach-O macOS binary.
- Five-expansion scope remains unchanged: Vanilla, TBC, WotLK, Cataclysm and MoP only in the UI.

# v1.6.14 — Startup Compatibility Repair

- Fixed the launch/Dock-bounce regression introduced by physically deleting post-MoP enum raw values.
- WoWCC still exposes and supports exactly five expansions: Vanilla, TBC, WotLK, Cataclysm, and MoP.
- Legacy expansion raw values remain internally decodable only for migration compatibility with older preferences/profile state. They are not shown in the UI and are not supported targets.
- Previously saved selections for removed expansions are automatically migrated to WotLK during startup instead of leaving an invalid expansion selection.
- Based on the known-working 1.6.11 application source plus the five-expansion scope change, minimizing startup-risk changes.
...
# v1.6.8 — WrathSilicon Client Prep + WotLK Build Watchdog
...
# v1.6.8 — WotLK Mounted Flying Everywhere
...
# v1.6.8 — Expansion Service Manager
